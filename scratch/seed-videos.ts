...
async function main() {
  try {
    ...
    if (!gallery) {
      ...
      console.log(`Created gallery: Interviews & TV`)
    } else {
      console.log(`Found gallery: ${gallery.name} (id: ${gallery.id})`)
    }
    ...
    for (let i = 0; i < VIDEO_ITEMS.length; i++) {
      ...
      if (exists) {
        console.log(`Skipped (exists): ${externalUrl}`)
        continue
      }
      ...
      await db.galleryItem.create({
        data: {
          ...
          type,
          section: section as import('@prisma/client').GallerySection, // Assuming section maps correctly to enum
          externalUrl,
          externalId,
          thumbnailUrl,
          isActive: true,
      ...
      console.log(`Created: [${type}] ${externalId ?? externalUrl}`)
    }
  } catch (err) {
